...
$plugins_data	= pbsm_get_plugin_data();
$plugins_filter = pbsm_plugins_filter();

// Check Plugin Install Permission
if( ! current_user_can('install_plugins') ) {
	echo '<div class="error">
			<p>'. esc_html__( "Sorry, It looks like that you do not have permission to install the plugin.", 'blog-manager-wp') .'</p>
			<p>'. esc_html__("You can take a look at our all plugins at", 'blog-manager-wp') .' <a href="https://profiles.wordpress.org/wpdiscover#content-plugins" target="_blank">'. esc_html__("here", 'blog-manager-wp') . '</a>.</p>
		 </div>';
	return;
}
?>
...
<div class="wrap pbsm-settings">
	<div class="pbsm-dashboard-wrap">

		<?php if( ! empty( $plugins_data->plugins ) ) { ?>
		<form id="plugin-filter" method="post">
			<div class="pbsm-plugin-list-wrap">
				<div class="widefat pbsm-plugin-list pbsm-clearfix row" id="the-list">
					...
					<div class="pbsm-plugin-card-wrap <?php echo $extra_class; ?> col-md-6" data-tags="<?php echo esc_attr( $plugin_tags ); ?>">
						<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin_data['slug'] ); ?>">
							<div class="plugin-card-top">
								<div class="name column-name">
									<h3>
										<a href="<?php echo esc_url( $details_link ); ?>" class="thickbox open-plugin-details-modal">
											<span class="pbsm-plugin-name"><?php echo $title; ?></span>
					...
					</div><!-- end .pbsm-plugin-card-wrap -->

					<?php } ?>

				</div>
				
			</div><!-- end .pbsm-plugin-list-wrap -->			
		</form>
		<?php } else { ?>

				<div class="pbsm-no-result">
...
function pbsm_get_plugin_data() {

	// Get cache result
	$plugins_data = get_transient( 'pbsm_plugins_data' );
	...
	if( empty( $plugins_data ) ) {
		...
		if( ! is_wp_error( $plugins_data ) && ! empty( $plugins_data->plugins ) ) {
			...
			usort( $plugins_data->plugins, "pbsm_sort_plugin_data" );

			set_transient( 'pbsm_plugins_data', $plugins_data, (12 * HOUR_IN_SECONDS) );
		}
	}
...
function pbsm_sort_plugin_data( $a, $b ) {
